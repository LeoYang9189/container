...
interface RepairWorkOrderPrintProps {
  data: {
    id: string;
    containerNo: string;
    yard: string;
...
const RepairWorkOrderPrint: React.FC<RepairWorkOrderPrintProps> = ({ data }) => {
  // 获取状态标签
  ...
  // 获取施工项目标签
  const getRepairItemLabel = (item: string) => {
  ...
  const getQRCodeValue = () => {
    ...
    return `${baseUrl}/containersaas/repair-work-orders/${data.id}`;
  };
    ...
    <div className="p-8" style={{ width: '210mm', minHeight: '297mm' }}>
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="text-center flex-1">
          <Title heading={2}>集装箱修理工作单</Title>
        ...
        <div className="mb-4">
          ...
          <div className="mt-2">
            {data.repairItems.map(item => (
              <Text key={item} className="mr-4">{getRepairItemLabel(item)}</Text>
            ))}
          </div>
          {data.otherRepairItem && (
            <div className="mt-2">
              <Text type="secondary">其他施工项目说明：</Text>
              <Paragraph className="mt-2">{data.otherRepairItem}</Paragraph>
            </div>
          )}
        </div>
        <div>
          <Text type="secondary">修理要求：</Text>
          <Paragraph className="mt-2">{data.repairRequirements}</Paragraph>
        </div>
      </div>
...
export default RepairWorkOrderPrint; 